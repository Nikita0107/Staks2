...
logging.basicConfig(level=logging.DEBUG, filename='py_log1.log',filemode='w', format='%(asctime)s %(levelname)s %(message)s')
...
while True:
    try:
        a, b, c = map(float, input('Введите коэффициенты для уравнения: a, b, c: ').split())
